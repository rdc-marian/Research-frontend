...
// Initial coordinator registry tabs
const DEFAULT_COORDINATOR_TABS = [
  { id: "scholars", label: "Research Scholars", isPredefined: true, columns: ["Sl.No.", "Scholar Name", "Research Topic", "Guide / Supervisor", "Admission Year", "Status"] },
  { id: "publications", label: "Approved Publications", isPredefined: true, columns: ["Sl.No.", "Publication Title", "Author (Scholar/Faculty)", "Journal Name", "Year of Publication", "Impact Factor"] },
  { id: "facilities", label: "Research Facilities", isPredefined: true, columns: ["Sl.No.", "Facility / Lab Name", "Equipment/Software", "Location", "Lab In-Charge"] },
  { id: "projects", label: "Funded Projects", isPredefined: true, columns: ["Sl.No.", "Project Title", "Principal Investigator", "Funding Agency", "Amount Sanctioned", "Status"] },
  { id: "guides", label: "Research Guides", isPredefined: true, columns: ["Sl.No.", "Guide Name", "Designation", "Area of Expertise", "No. of Guided Scholars"] },
];

const DEFAULT_COORDINATOR_TABS_DATA: Record<string, any[]> = {
  scholars: [],
  publications: [],
  facilities: [],
  projects: [],
...
export default function CoordinatorDashboard() {
  ...
  const toggleTabCheckbox = (tabId: string) => {
    ...
    if (user?._id) {
      const updatedPrefs = {
        ...(user.preferences || {}),
        coordinator_active_tabs: nextActive
      };
  ...
  const handleCreateCustomTab = () => {
    ...
    if (user?._id) {
      const updatedPrefs = {
        ...(user.preferences || {}),
        coordinator_tabs_config: nextList,
        coordinator_active_tabs: nextActive,
        coordinator_custom_tabs_data: nextData
      };
      apiPatchJson(`/users/${user._id}`, { preferences: updatedPrefs }).then((res: any) => {
        if (res?.item) login("", res.item);
      });
    ...
    >
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex-1 space-y-1.5 w-full">
            ...
            <div className="pt-2 text-xs space-y-1.5 text-slate-700 grid grid-cols-1 md:grid-cols-2 gap-x-4">
              ...
              <div>
                <span className="font-semibold text-slate-500">Oversight Center : </span>
                <span className="font-bold text-slate-800">{profileCenter}</span>
              </div>
              <div>
                <span className="font-semibold text-slate-500">Academic Year : </span>
                <span className="font-bold text-slate-800">{profileAcademicYear}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-[color:var(--border)] bg-white p-6 shadow-sm mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[color:var(--border)] pb-3 mb-6 gap-3">
          <h2 className="font-display text-lg font-bold text-[#9B0302]">
            Research Center Portfolio Registry
          </h2>
        ...
        <div className="flex items-center justify-between gap-4 border-b border-[color:var(--border)] pb-4">
          <div>
            <h2 className="font-display text-lg text-[color:var(--maroon-900)]">
              {profileDept} submissions
            </h2>
            <p className="text-sm text-slate-500">
              Recent submissions across the {profileDept} Research Center.
            </p>
          </div>
      ...
      {showEditProfileModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-in fade-in duration-200">
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl border border-[color:var(--border)] animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-[color:var(--border)] pb-3">
              <h3 className="font-display text-base font-bold text-[#9B0302]">
                Edit Coordinator & Research Center Details
              </h3>
              <button
            ...
            <div className="mt-4 space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">Oversight Center</label>
                  <input
                    type="text"
                    value={profileCenter}
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">Academic Year</label>
                  <input
                    type="text"
      ...
      {showConfigModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-[color:var(--border)] animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-[color:var(--border)] pb-3">
              <h3 className="font-display text-base font-bold text-[#9B0302]">
                Configure Portfolio Tabs
              </h3>
              <button
            ...
            <p className="mt-2 text-xs text-slate-500">
              Select which sections to display on the dashboard.
            </p>
      ...
      {showAddTabModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 animate-in fade-in duration-200">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl border border-[color:var(--border)] animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-[color:var(--border)] pb-3">
              <h3 className="font-display text-base font-bold text-[#9B0302]">
                Create Custom Tab
              </h3>
